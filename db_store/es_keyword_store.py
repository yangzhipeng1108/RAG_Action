...
es.indices.delete(index='es_subkeyword_store', ignore=[400, 404])
#
es.indices.create(index="es_subkeyword_store",ignore=400)
...
for key in Subdocument_result_dict.keys():
    if len(Subdocument_result_dict[key]) > 1:
        ...
        for  context in Subdocument_result_dict[key]:
            doc_inter.append({"_index":"es_subkeyword_store","_type":"doc","_id":i,"_source":{"title":key,"keyword":context}})
            i += 1
        action.extend(doc_inter)
    else:
        action.append({"_index":"es_subkeyword_store","_type":"doc","_id":i,"_source":{"title":key,"keyword":Subdocument_result_dict[key][0]}})
        i += 1
...
es.indices.delete(index='es_keyword_store', ignore=[400, 404])
es.indices.create(index="es_keyword_store",ignore=400)
...
for key in result_dict.keys():
    action.append({"_index":"es_keyword_store","_type":"doc","_id":i,"_source":{"title":key,"keyword":result_dict[key]}})
    i += 1
