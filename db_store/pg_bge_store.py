...
cur.execute('CREATE TABLE subdocument (id bigserial PRIMARY KEY,title text , title_embedding vector(1024),content text,content_embedding vector(1024))')
...
for key in Subdocument_result_dict.keys():
    if len(Subdocument_result_dict[key]) > 1:
        ...
        for title,title_embedding,content, content_embedding in zip(key_list, key_embeddings,Subdocument_result_dict[key],value_embeddings):
            ...
            cur.execute('INSERT INTO subdocument (title,title_embedding,content, content_embedding) VALUES (%s, %s,%s, %s)' , (title,str(list(title_embedding)),content, str(list(content_embedding))))
    else:
        title_embedding = model.encode([key])

        content_embedding =  model.encode(Subdocument_result_dict[key])

        cur.execute('INSERT INTO subdocument (title,title_embedding,content, content_embedding) VALUES (%s, %s,%s, %s)', (key,str(list(title_embedding[0])),Subdocument_result_dict[key][0], str(list(content_embedding[0]))))

cur.execute('CREATE INDEX  ON subdocument USING ivfflat(content_embedding vector_cosine_ops) WITH(lists = 1)')


cur.execute('DROP TABLE IF EXISTS documents')
cur.execute('CREATE TABLE documents (id bigserial PRIMARY KEY,title text , title_embedding vector(1024), content text, content_embedding vector(1024))')
...
for key in result_dict.keys():
    ...
    cur.execute('INSERT INTO documents (title,title_embedding,content, content_embedding) VALUES (%s, %s,%s, %s)'
