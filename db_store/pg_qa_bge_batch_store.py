...
cur.execute(
    'CREATE TABLE qa_store (id bigserial PRIMARY KEY,question text , question_embedding vector(1024), answer text, answer_embedding vector(1024))')
...
# 构造INSERT语句
insert_document_query = "INSERT INTO qa_store (question,question_embedding,answer, answer_embedding)  VALUES %s"

# 使用mogrify方法构造批量插入语句
psycopg2.extras.execute_values(cur, insert_document_query, insert_document_data)
cur.execute('CREATE INDEX  ON qa_store USING ivfflat(question_embedding vector_cosine_ops) WITH(lists = 1)')
conn.commit()
