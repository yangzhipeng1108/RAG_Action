...
cur.execute('DROP TABLE IF EXISTS qa_store')
cur.execute('CREATE TABLE qa_store (id bigserial PRIMARY KEY,question text , question_embedding vector(1024), answer text, answer_embedding vector(1024))')
...
for key in result_dict.keys():
    ...
    cur.execute('INSERT INTO qa_store (question,question_embedding,answer, answer_embedding) VALUES (%s, %s,%s, %s)'
                 , (key,str(list(title_embedding)),result_dict[key], str(list(content_embedding))))

cur.execute('CREATE INDEX  ON qa_store USING ivfflat(answer_embedding vector_cosine_ops) WITH(lists = 1)')
conn.commit()
# 关闭游标
cur.close()
